...
const ExperienceSection: React.FC = () => {
  ...
  return (
    <section id="experience" ref={ref}>
      ...
      <div className={`grid gap-4 my-12 lg:gap-x-6 opacity-0 ${isVisible && animationStyles.slideUp}`}>  
        {
          EXPERIENCE.map((data: ExperienceTypes, key: number) => (
            <div key={key}>    
              <div className={`${styles.cardExperience} shadow-md md:shadow-none hover:shadow-md`}>
                <div className="py-4 grid md:grid-cols-[20%_1fr]">
                  <div className="text-[#808080] text-[12px] uppercase py-1">{data.year}</div>
                  <div>
                    <div className={`${styles.title} font-semibold md:text-[20px] leading-[22px] gap-1 md:gap-2 space-x-2`}>
                      <span>{data.title}</span>
                      <span>·</span>
                      <span>{data.company}</span>
                    </div>
                
                    {data.subtitle && <div className="mt-4">{data.subtitle}</div>}
